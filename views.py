...
from healthie.health_api import *
from healthie.health_input import *

# def index(request):
#     return render(request, 'healthie/index.html')
...
def index(request):
    ...
    if request.method == "GET":
        ...
        if form.is_valid():
            ...
            print("RESTRICTIONS:", p.restrictions)
            p.update_info()
            return foodresults(request, p)
            # return HttpResponseRedirect('')
            # return render(request, 'healthie/enterinfo.html', {'form': form, 'PageTitle': 'Enter Your Info', 'entered': True})
        print("\t\t\t\tERRORS: ", form.errors)
    # print("Input not valid.")
    form = CalculateCaloriesForm()
    return render(request, 'index.html', {'form': form, 'PageTitle': 'Dang it!'})
